...
#' @param huber If not NA (default), the Huber weight used to control outliers.
...
#' @importFrom stats qt
#' @export
Rodionov <- function(data, col, time, l, prob = 0.05, startrow = 1, merge = FALSE, huber=NA){
  vals <- data[[col]]
  t_crit <- abs(qt(prob/2, (2 * l - 2)))  # the fiddling is to get two-tailed test

  # call Rust code
  if (is.na(huber)) {
    RSI <- rust_rodionov(vals, t_crit, l, rep(1, length(vals)))
  } else {
    RSI <- rust_rodionov_huber(vals, t_crit, l, huber)
  }
